...
状态：NEEDS_RESEARCH。R011 的作用方程接口保留；五种子实验否定 R012 的余弦充分门控，当前进入 R013 增量 guard 设计。

## 0. 实验证据后的边界

2026-09-06 的已知映射实验表明，Functional Map 可以在干净系统中运输 action pairs，接收方也能把正确作用的留出缺陷降低约 `94%--96%`。但“映射可信、缺陷非零且任务/方程梯度余弦为正”不是充分的流动条件：它会接收 local-only 已能掌握的冗余方程，也会阶段性接收 source/time-shuffled 方程。后者即使最终流量归零，仍已造成任务损害。

因此本文第 5 节的余弦电导是 R012 的被否定候选，只能作为 R013 的预筛选。最终接受必须比较独立 task guard 上的 matched local-only 与 local-plus-action 增量收益。完整结果见 `.collab/EXPERIMENTS.md` 的 E019。
...
R012 曾用映射置信度、任务相容性和缺陷阈值共同决定边电导
