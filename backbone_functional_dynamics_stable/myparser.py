...
class Parser:
    ...
    def set_arguments(self):
        ...
        parser.add_argument(
            '--enable-functional-dynamics', action='store_true',
            help='Enable cached functional-dynamics translation and injection.',
        )
        parser.add_argument('--fd-rank', type=int, default=8)
        parser.add_argument('--fd-probe-dim', type=int, default=4)
        parser.add_argument('--fd-ridge-lambda', type=float, default=1e-3)
        parser.add_argument('--fd-fm-steps', type=int, default=10)
        parser.add_argument('--fd-fm-lr', type=float, default=0.1)
        parser.add_argument('--fd-lambda-desc', type=float, default=1.0)
        parser.add_argument('--fd-lambda-dyn', type=float, default=1.0)
        parser.add_argument('--fd-beta', type=float, default=0.05)
        parser.add_argument('--fd-spectral-limit', type=float, default=1.0)
        parser.add_argument('--fd-ema-a', type=float, default=0.9)
        parser.add_argument('--fd-ema-b', type=float, default=0.9)
        parser.add_argument(
            '--fd-num-prototypes', type=int, default=1,
            help='Number of canonical dynamics regimes; 1 preserves the single-prototype method.',
        )
        parser.add_argument(
            '--fd-cluster-switch-margin', type=float, default=0.05,
            help='Relative Procrustes-distance improvement required to switch regimes.',
        )
        parser.add_argument('--fd-min-cluster-size', type=int, default=2)
        parser.add_argument('--fd-map-type', choices=['orthogonal', 'regularized'], default='orthogonal')
        parser.add_argument('--fd-map-ridge', type=float, default=1e-3)
        parser.add_argument('--fd-map-orth-reg', type=float, default=1e-2)
        parser.add_argument('--fd-map-max-condition', type=float, default=20.0)
        parser.add_argument(
            '--fd-normalize-generator', action='store_true',
            help='Align and aggregate dimensionless generator shape, restoring each client speed on pullback.',
        )
