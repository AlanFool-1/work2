...
def set_config(args):
    ...
    if int(args.fd_rank) <= 0 or int(args.fd_probe_dim) <= 0:
        raise ValueError('Functional rank and probe dimension must be positive.')
    if int(args.fd_rank) > 4 * int(args.fd_probe_dim):
        raise ValueError('fd_rank must be <= 4 * fd_probe_dim.')
    if float(args.fd_ridge_lambda) <= 0.0:
        raise ValueError('fd_ridge_lambda must be positive.')
    if int(args.fd_fm_steps) <= 0 or float(args.fd_fm_lr) <= 0.0:
        raise ValueError('Functional-map optimization settings must be positive.')
    if min(float(args.fd_beta), float(args.fd_spectral_limit)) < 0.0:
        raise ValueError('Injection beta and spectral limit must be non-negative.')
    if not 0.0 <= float(args.fd_ema_a) < 1.0:
        raise ValueError('fd_ema_a must be in [0,1).')
    if not 0.0 <= float(args.fd_ema_b) < 1.0:
        raise ValueError('fd_ema_b must be in [0,1).')
    if int(args.fd_num_prototypes) <= 0 or int(args.fd_num_prototypes) > int(args.n_clients):
        raise ValueError('fd_num_prototypes must be in [1,n_clients].')
    if float(args.fd_cluster_switch_margin) < 0.0:
        raise ValueError('fd_cluster_switch_margin must be non-negative.')
    if int(args.fd_min_cluster_size) <= 0:
        raise ValueError('fd_min_cluster_size must be positive.')
    if int(args.fd_min_cluster_size) * int(args.fd_num_prototypes) > int(args.n_clients):
        raise ValueError('Minimum cluster sizes exceed the client count.')
    if min(float(args.fd_map_ridge), float(args.fd_map_orth_reg)) < 0.0:
        raise ValueError('Functional-map regularizers must be non-negative.')
    if float(args.fd_map_max_condition) < 1.0:
        raise ValueError('fd_map_max_condition must be at least 1.')
