...
def validate_official_source_and_gradients():
    model = build_model()
    expected_source = (
        ROOT.parent / 'Anti-SymmetricDGN-main' / 'graph_heteropily'
        / 'models' / 'antisymmetric_dgn.py'
    ).resolve()
...
def validate_shared_field_euler_iterations():
    ...
    data = ring_graph()
    states = model.encode_dynamics_states(data)
    assert states.shape == (5, 12, 8)
    assert torch.equal(states[0], model.encode_initial_state(data))
