...
**实验状态（2026-09-06）：R012 当前门控未通过。** 五种子已知映射实验支持 Functional Map 运输和 action-pair distillation，但否定“交换缺陷非零且任务/方程梯度余弦为正”是充分流动条件。structure-only 中 local-only 已经足够，余弦仍错误地打开冗余流；source/time shuffle 也会阶段性通过余弦门控并造成任务损害。以下作用流方程保留为被测骨架，不得解释为已经验证的最终算法。
...
R013 必须把 \([\chi]_+\) 从充分门控降为预筛选，并在独立本地 guard 上比较一次等范数 local-only 更新与 local-plus-action 更新。只有外部候选产生正增量 guard 收益时才允许流动。该反事实可以低频执行，但不能再由梯度余弦替代，因为本轮实验已经证明“方向不冲突”不等于“外部知识有新增价值”。
