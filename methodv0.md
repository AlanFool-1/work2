...
在 feature-only、structure-only 和 joint synthetic 场景的早、中、晚训练 checkpoint 上，分别从同一快照执行：
...
逐客户端报告预测 $G_i$、实际 $\widehat G_i$、外部更新范数、响应实现残差和任务变化。核心预测是：真实 external update 在早期具有正增益，随后 $\widehat G_i$ 与接受率逐步下降并趋近零；打乱响应不应复现该曲线。若外部增益不随训练下降、乱配响应同样有效，或停止流动只由人工 clipping 造成，则“有限互补知识吸收”主张不成立。
...
公共 probe 响应本身直接作为 v0 的知识状态，不需要先拟合生成器。只有在通信成本过高、需要压缩 \(x_m\) 时，才比较以下两个凸压缩模型，并把直接离散流算子作为默认候选：
...
\widehat A_m
=\arg\min_A
\sum_{t\in\mathcal I_{\mathrm{fit}}}
\left\|
\frac{z_{t+1}-z_t}{\Delta t}-Az_t
\right\|_2^2
 +\lambda_A\|A\|_F^2.
\]
...
两者都是凸二次问题，可用 ridge 线性方程求解。第二个模型不需要用 \(\Delta t\) 放大差分噪声，且直接拟合实际 Euler 传播；运输时使用 \(C_mP_mC_m^{-1}\)，回写时使用 \((P_m-I)/\Delta t\) 或直接使用离散修正。若初始状态存在系统性偏移，再比较带偏置的凸模型：
...
它能表达非线性，但目标是非凸的，容易在短轨迹上过拟合，也没有天然的跨客户端坐标运输和守恒解释。v0 不交换神经网络参数；只有当神经模型在时间外验证上显著优于两个凸压缩模型时，才把它作为“线性低维状态不足”的证据，并另行研究可运输的神经特征。第一轮优先直接交换 probe 响应，离散 \(P_m\) 仅作为通信压缩对照，因为前者最忠实于实际有限时间图动力学。
